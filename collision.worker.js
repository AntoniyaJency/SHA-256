// Worker: search for two distinct messages whose hashes share the first N bits.
// Supports algo: 'SHA-256' (WebCrypto) and 'MD5' (JS implementation sent via postMessage? we re-implement minimal MD5 here for isolation).

// Minimal MD5 (same as main thread, duplicated to avoid dependency)
function md5(str){
...
function prefixBits(hex, bits){
  const bytesNeeded = Math.ceil(bits/8);
  const sliced = hex.slice(0, bytesNeeded*2);
  const val = parseInt(sliced, 16);
  const extra = (bytesNeeded*8 - bits);
  return val >>> extra;
}
...
onmessage = async (e) => {
  ...
  const { algo, bits, base } = e.data;
  running = true;
  const seen = new Map();
  let attempts = 0;
  let lastProgress = 0;
  while (running) {
    const a = base + Math.random().toString(36).slice(2);
    const b = base + Math.random().toString(36).slice(2);
    const m = (attempts % 2 === 0) ? a : b; // vary
    let hex;
    ...
    attempts++;
    if (attempts - lastProgress > 1000) {
      lastProgress = attempts;
      postMessage({ type: 'progress', algo, attempts, seen: seen.size });
    }
  }
};
